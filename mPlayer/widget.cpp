...
const int kMaxPacketSize = 15*1024*1024;
...
int Widget::decodeFile()
{
    LogInit();

    char fileName[128] = {"/home/wlw/Desktop/mPlayer/media/1.mp4"};
    ...
    qDebug()<< "[audioStream]->time_base:"<< pFormatCtx->streams[audioStream]->time_base.num << " "<< pFormatCtx->streams[audioStream]->time_base.den;
    qDebug()<< "[videoStream]->time_base:"<< pFormatCtx->streams[videoStream]->time_base.num << " "<< pFormatCtx->streams[videoStream]->time_base.den;
    ...
    while (eof != 1)
    {
        ...
        if(av_read_frame( pFormatCtx, &packet ) >= 0)
        {
            ...
            if ( packet.stream_index == videoStream )           // 视频包
            {
                ...
                LogDebug("video pts: %ld", packet.pts);
